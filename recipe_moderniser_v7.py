"""Recipe moderniser - full working program (version 7)
Improving the spacing and readability of the output - adds a rounding function
to eliminate impractical fractional amounts
...
def round_amount(amount_to_round):
    if amount_to_round % 1:  # Finds whole numbers
        ...
        amount_to_round = "{:.2f}".format(amount_to_round)  # Everthing else is rounding to 2dp
    ...
    if num_spaces > 1:  # Item has both unit and ingredient
        ...
        if amount[1]:
            ...
            # If the ingredinet is not in the food dictionary, leave as ml
                ...
                modernised_recipe.append("{:.0f} ml {}".format(amount_2[0], ingredient))
